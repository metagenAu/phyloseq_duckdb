...
#' @export
#' DuckDB-based sample data class
#'
#' @slot con DuckDB connection
#' @exportClass sample_data_duckdb
setClass("sample_data_duckdb",
         slots = c(
           con = "DBIConnection"
         ))
...
#' @param object A data.frame or existing sample_data object
#' @param db_path Path to store the DuckDB database. Default is temporary
#' @return A sample_data_duckdb object
#' @examples
#' # Create from data frame
#' sample_df <- data.frame(
#'   sample_id = paste0("Sample", 1:5),
...
sample_data_duckdb <- function(object, db_path = tempfile()) {
    ...
    new("sample_data_duckdb", con = con)
}

#' Close DuckDB connection for sample data
#'
#' @param x A sample_data_duckdb object
#' @export
close_sample_data_duckdb <- function(x) {
    if (!is(x, "sample_data_duckdb")) {
...
#' @return Filtered sample_data_duckdb object
#' @examples
...
filter_sample_data_duckdb <- function(x, variable, value, condition = "=") {
    ...
    DBI::dbExecute(x@con, "DROP TABLE sample_data")
    DBI::dbExecute(x@con, "ALTER TABLE sample_data_filtered RENAME TO sample_data")
    
    # Recreate index
    DBI::dbExecute(x@con, "CREATE INDEX idx_sample_id ON sample_data(sample_id)")
...
#'
#' @param sample_data A sample_data_duckdb object
#' @param otu_table An otu_table_duckdb object
...
merge_sample_otu_duckdb <- function(sample_data, otu_table) {
    if (!is(sample_data, "sample_data_duckdb") || !is(otu_table, "otu_table_duckdb")) {
        stop("Inputs must be sample_data_duckdb and otu_table_duckdb objects")
    }
    
    # Create merged view
    DBI::dbExecute(otu_table@con, "
